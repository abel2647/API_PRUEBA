...
public interface RegistroEntradaVisitanteRepository extends JpaRepository<RegistroEntradaVisitanteModel, Integer> {
    ...
    // MÉTODOS HISTÓRICOS (Puedes dejarlos si los usas en otro lado, pero ya no se usarán en este Excel)
    @Query("SELECT COUNT(r) FROM RegistroEntradaVisitanteModel r WHERE r.codigoTemporal.visitante.id = :visitanteId")
    long countTotalVisitasPorVisitante(@Param("visitanteId") Integer visitanteId);

    @Query("SELECT MAX(r.fechaHora) FROM RegistroEntradaVisitanteModel r WHERE r.codigoTemporal.visitante.id = :visitanteId")
    LocalDateTime findUltimaVisitaPorVisitante(@Param("visitanteId") Integer visitanteId);
}